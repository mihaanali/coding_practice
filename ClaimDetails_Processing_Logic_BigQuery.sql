/*
================================================================================
  CLAIMS PROCESSING LOGIC FOR `prominence.ClaimDetails`
...
================================================================================
  ...
  BIGQUERY NOTES:
  ---------------
  - Replace `prominence.ClaimDetails` with your actual `project.dataset.table`
  - Views are created with CREATE OR REPLACE VIEW
  - ISNULL() replaced with IFNULL()
  - GETDATE() replaced with CURRENT_TIMESTAMP()
  - DATEDIFF(DAY, a, b) replaced with DATE_DIFF(b, a, DAY)
  - SELECT INTO replaced with CREATE TABLE AS SELECT
  - GO batch separators removed
  - Indexes replaced with CLUSTER BY / PARTITION BY on table creation
  - Correlated EXISTS replaced with LEFT JOIN for BigQuery performance
  ...
  8. MasterClaimId = claimid for originals; different for adjustments.
     orgclaimid points back to the original claim being adjusted.
...
-- ============================================================================
CREATE OR REPLACE VIEW `prominence.vw_ClaimDetails_Normalized` AS
SELECT
    cd.*,
    ...
    CASE
        WHEN cd.claimid IN (
            SELECT adj.orgclaimid
            FROM `prominence.ClaimDetails` adj
            WHERE adj.orgclaimid IS NOT NULL
              AND adj.claimid <> adj.orgclaimid
        ) THEN 1
        ELSE 0
    END AS IsSuperseded,

    -- Calculated: implicit contractual write-off (billed - allowed)
    IFNULL(claimamt_or_Charges, 0) - IFNULL(AllowedAmt_or_ContractPaid, 0) AS ImpliedContractualDiscount,

    -- Calculated: Medicare sequestration amount (2% of allowed for MA claims)
    CASE
        WHEN Company = 'MEDICARE ADVANTAGE'
         AND IFNULL(AllowedAmt_or_ContractPaid, 0) > 0
         AND IFNULL(amountpaid, 0) > 0
        THEN ROUND(AllowedAmt_or_ContractPaid * 0.02, 2)
        ELSE 0
    END AS EstimatedSequestration

FROM `prominence.ClaimDetails` cd;
...
CREATE OR REPLACE VIEW `prominence.vw_FinalClaimState` AS
WITH RankedClaims AS (
    ...
    FROM `prominence.ClaimDetails`
    WHERE ClaimStatus NOT IN ('VOID', 'REV')
...
SELECT
    ...
    IFNULL(claimamt_or_Charges, 0)        AS FinalBilledAmount,
    IFNULL(AllowedAmt_or_ContractPaid, 0) AS FinalAllowedAmount,
    IFNULL(amountpaid, 0)                 AS FinalPaidAmount,
    IFNULL(copay, 0)                      AS FinalCopay,
    IFNULL(coinsuranceamt, 0)             AS FinalCoinsurance,
    IFNULL(Deductible, 0)                 AS FinalDeductible,
    IFNULL(memamt, 0)                     AS FinalMemberAmount,
    IFNULL(COB_or_extpaidAmt, 0)          AS FinalCOBAmount,
    IFNULL(costshareamt, 0)               AS FinalCostShare,

    IFNULL(claimamt_or_Charges, 0) - IFNULL(AllowedAmt_or_ContractPaid, 0) AS ContractualDiscount,
    IFNULL(copay, 0) + IFNULL(coinsuranceamt, 0) + IFNULL(Deductible, 0)  AS CalcMemberResponsibility,
    IFNULL(amountpaid, 0) + IFNULL(copay, 0) + IFNULL(coinsuranceamt, 0)
        + IFNULL(Deductible, 0) + IFNULL(COB_or_extpaidAmt, 0)            AS CalcTotalAccountedFor,
...
-- ============================================================================
CREATE OR REPLACE VIEW `prominence.vw_ClaimNetEffect` AS
SELECT
    MasterClaimId,
    claimline,
    memid,
    ...
    SUM(IFNULL(claimamt_or_Charges, 0))                  AS NetBilledAmount,
    SUM(IFNULL(AllowedAmt_or_ContractPaid, 0))           AS NetAllowedAmount,
    SUM(IFNULL(amountpaid, 0))                           AS NetPaidAmount,
    SUM(IFNULL(copay, 0))                                AS NetCopay,
    SUM(IFNULL(coinsuranceamt, 0))                       AS NetCoinsurance,
    SUM(IFNULL(Deductible, 0))                           AS NetDeductible,
    SUM(IFNULL(COB_or_extpaidAmt, 0))                    AS NetCOBAmount
...
SELECT
    ...
    IFNULL(amountpaid, 0)
        + IFNULL(copay, 0)
        + IFNULL(coinsuranceamt, 0)
        + IFNULL(Deductible, 0)
        + IFNULL(COB_or_extpaidAmt, 0)         AS TotalAccountedFor,

    IFNULL(AllowedAmt_or_ContractPaid, 0)
        - IFNULL(amountpaid, 0)
        - IFNULL(copay, 0)
        - IFNULL(coinsuranceamt, 0)
        - IFNULL(Deductible, 0)
        - IFNULL(COB_or_extpaidAmt, 0)         AS UnaccountedGap,
    ...
    CASE
        WHEN IFNULL(AllowedAmt_or_ContractPaid, 0) = 0 THEN 'NO_ALLOWED'
        WHEN ABS(
            IFNULL(AllowedAmt_or_ContractPaid, 0)
            - IFNULL(amountpaid, 0) - IFNULL(copay, 0)
            - IFNULL(coinsuranceamt, 0) - IFNULL(Deductible, 0)
            - IFNULL(COB_or_extpaidAmt, 0)
        ) < 0.50 THEN 'BALANCED'
        WHEN Company = 'MEDICARE ADVANTAGE'
         AND ABS(
...
ORDER BY
    CASE
        WHEN ABS(IFNULL(AllowedAmt_or_ContractPaid, 0) - IFNULL(amountpaid, 0)
             - IFNULL(copay, 0) - IFNULL(coinsuranceamt, 0) - IFNULL(Deductible, 0)
             - IFNULL(COB_or_extpaidAmt, 0)) > 0.50 THEN 0 ELSE 1
    END,
    ABS(IFNULL(AllowedAmt_or_ContractPaid, 0) - IFNULL(amountpaid, 0)) DESC;
...
FROM `prominence.vw_FinalClaimState`
GROUP BY
...
SELECT
    ...
    SUM(IFNULL(claimamt_or_Charges, 0))                          AS FinalBilledAmount,
    SUM(IFNULL(AllowedAmt_or_ContractPaid, 0))                   AS FinalAllowedAmount,
    SUM(IFNULL(amountpaid, 0))                                   AS FinalPlanPaidAmount,
    SUM(IFNULL(copay, 0) + IFNULL(coinsuranceamt, 0)
        + IFNULL(Deductible, 0))                                 AS FinalMemberResponsibility,
    SUM(IFNULL(copay, 0))                                        AS FinalCopay,
    SUM(IFNULL(coinsuranceamt, 0))                               AS FinalCoinsurance,
    SUM(IFNULL(Deductible, 0))                                   AS FinalDeductible,
    SUM(IFNULL(COB_or_extpaidAmt, 0))                            AS FinalCOB,
    SUM(IFNULL(claimamt_or_Charges, 0))
        - SUM(IFNULL(AllowedAmt_or_ContractPaid, 0))             AS FinalContractualWriteOff,

    SUM(IFNULL(amountpaid, 0) + IFNULL(copay, 0)
        + IFNULL(coinsuranceamt, 0) + IFNULL(Deductible, 0)
        + IFNULL(COB_or_extpaidAmt, 0))                         AS TotalAccountedFor,

    SUM(IFNULL(AllowedAmt_or_ContractPaid, 0))
        - SUM(IFNULL(amountpaid, 0) + IFNULL(copay, 0)
              + IFNULL(coinsuranceamt, 0) + IFNULL(Deductible, 0)
              + IFNULL(COB_or_extpaidAmt, 0))                   AS UnaccountedGap,
...
SELECT
    ...
    logdate,
    DATE_DIFF(CURRENT_DATE(), SAFE.PARSE_DATE('%m/%d/%Y', logdate), DAY) AS DaysSinceReceived,

    CASE
        WHEN DATE_DIFF(CURRENT_DATE(), SAFE.PARSE_DATE('%m/%d/%Y', logdate), DAY) > 30
        THEN 'OVERDUE'
        ELSE 'WITHIN_SLA'
...
WHERE ClaimStatus = 'PEND'
  AND (ClaimLevelOnly IS NULL OR ClaimLevelOnly = '' OR ClaimLevelOnly = '0')
ORDER BY
    DATE_DIFF(CURRENT_DATE(), SAFE.PARSE_DATE('%m/%d/%Y', logdate), DAY) DESC;
...
SELECT
    ...
    SUM(IFNULL(claimamt_or_Charges, 0))                         AS TotalBilled,
    SUM(IFNULL(AllowedAmt_or_ContractPaid, 0))                  AS TotalAllowed,
    SUM(IFNULL(amountpaid, 0))                                  AS TotalPlanPaid,
    SUM(IFNULL(copay, 0) + IFNULL(coinsuranceamt, 0)
        + IFNULL(Deductible, 0))                                AS TotalMemberResp,
    SUM(IFNULL(COB_or_extpaidAmt, 0))                           AS TotalCOB,
    SUM(IFNULL(claimamt_or_Charges, 0))
        - SUM(IFNULL(AllowedAmt_or_ContractPaid, 0))            AS TotalContractualDiscount,

    CASE WHEN SUM(IFNULL(claimamt_or_Charges, 0)) > 0
         THEN ROUND(SUM(IFNULL(AllowedAmt_or_ContractPaid, 0))
              / SUM(IFNULL(claimamt_or_Charges, 0)) * 100, 1)
    END AS AllowedToBilledPct,

    CASE WHEN SUM(IFNULL(AllowedAmt_or_ContractPaid, 0)) > 0
         THEN ROUND(SUM(IFNULL(amountpaid, 0))
              / SUM(IFNULL(AllowedAmt_or_ContractPaid, 0)) * 100, 1)
...
-- ============================================================================
-- In BigQuery, use CREATE OR REPLACE TABLE instead of SELECT INTO.
-- Clustering replaces traditional indexes for query performance.
-- Consider scheduling this as a BigQuery scheduled query to keep it current.

-- DROP TABLE IF EXISTS `prominence.ClaimDetails_Final`;

CREATE OR REPLACE TABLE `prominence.ClaimDetails_Final`
CLUSTER BY FinalClaimStatus, memid, RenderingProviderNPI, StartServiceDate
AS
SELECT
    fc.MasterClaimId,
    fc.claimid,
...
FROM `prominence.vw_FinalClaimState` fc;
